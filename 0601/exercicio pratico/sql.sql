...
-- Dump completed on 2018-11-26 17:06:05cidadecidade


select * from filial;

-- 1 Recuperar o nome e telefone dos clientes, o número de contrato de locação de veículo, além do modelo eplaca do veículo. Ordene os contratos pela data de saída mais recente
SELECT c.nome, t.numero, ct.numero, ct.dataSaida, v.placa, v.modelo
FROM cliente c
INNER JOIN contrato ct ON c.codigo = ct.codigoCliente
INNER JOIN telefone t ON c.codigo = t.codigoCliente
INNER JOIN veiculo v ON ct.idVeiculo = v.id
...
SELECT c.nome AS cidade, REPLACE(c.anofundacao, '.', '') AS anofundacao, COUNT(v.id) AS quantidade_veiculos
FROM cidade c
LEFT JOIN filial f ON c.codigo = f.idCidade
LEFT JOIN veiculo v ON f.codigo = v.codFilial
GROUP BY c.nome
HAVING quantidade_veiculos > 0
ORDER BY quantidade_veiculos ASC;

--  3. . Mostrar todos os contratos firmados pelo cliente “Filomena” sobre o veículo de placa “'MAB0002'”. Mostraro “Nome do cliente”, “Número do contrato”, “Placa do veículo”, “Modelo do veículo”, “Data saída” e “Datadevolução”
SELECT c.nome AS 'Nome cliente', co.numero AS 'Número do contrato', v.placa AS 'Placa do veículo',
       v.modelo AS 'Modelo do veículo', co.datasaida AS 'Data saída', co.datadevolucao AS 'Data devolução'
FROM cliente c
JOIN contrato co ON c.codigo = co.codigoCliente
JOIN veiculo v ON co.idVeiculo = v.id
WHERE c.nome = 'Filomena' AND v.placa = 'MAB0002';


-- 4. Exibir a placa e o modelo de todos os veículos de 4 portas das filiais cuja população seja inferior a cem milhabitantes. Exibir também os nomes das cidades das filiais e as Ruas
SELECT c.nome AS cidade, f.rua, v.placa, v.modelo, v.numPortas
FROM veiculo v
JOIN filial f ON v.codFilial = f.codigo
JOIN cidade c ON f.idCidade = c.codigo
WHERE v.numPortas = 4 AND c.populacao < 100000;

-- 5.  Mostrar a quantidade de veículos com diárias maiores que 100 reais e registrados em filiais com populaçãomaior que 500 mil habitantes. Mostre o modelo do veículo e a quantidade
...
SELECT v.placa, v.modelo, AVG(c.valorPago) AS media_valor
FROM contrato c
JOIN veiculo v ON c.idVeiculo = v.id
GROUP BY v.placa, v.modelo
ORDER BY media_valor DESC;

-- 7. Mostrar a quantidade de veículos com diárias maiores que 100 reais e registrados em filiais com populaçãomaior que 500 mil habitantes. Mostre o modelo do veículo e a quan
...
-- 8. Exiba o modelo e a placa dos veículos que ainda foram locados na empresa
SELECT v.modelo,v.placa
FROM veiculo v
LEFT JOIN contrato c ON c.idVeiculo = v.id
WHERE c.idVeiculo IS NULL;
